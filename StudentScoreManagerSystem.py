...
class PyMySQLUtils:
    ...
    def __init__(self):
        # 打开数据库连接
        self.db = pymysql.connect(host="localhost", user="root", passwd="13187694129", db="school")
        # 使用cursor()方法获取操作游标
...
class TeacherMenu:
    def __init__(self, parent_window):
        ...
        Button(self.frame_right, text="写入到Excel文件", width=30,
               command=lambda: mysql_excel("StudentScore.xlsx")).grid(row=5, column=0, padx=5, pady=5)
